...
namespace SmoothReflections
{
    ...
    {
        #region isinst hack and XOR hack
        ...
        public static CodeExpression Xor(CodeExpression left, CodeExpression right)
        {
            var provider = (CSharpCodeProvider)CodeDomProvider.CreateProvider("csharp");

            return
                new CodeSnippetExpression("(" + ExpressionToString(left, provider) + "^" + ExpressionToString(right, provider) + ")");
        }
        ...
        public static string ToCsharp(CodeTypeDeclaration type)
        {
            var csharpGenerator = (CSharpCodeProvider)CodeDomProvider.CreateProvider("csharp");
            var output = new StringBuilder();

            using(var writer = new StringWriter(output))
            {
                csharpGenerator.GenerateCodeFromType(type, writer,
